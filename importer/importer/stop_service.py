...
from settings import (
    logging,
    COVERED_DISTANCE_INSIDE_STOP_THRESHOLD,
    DISTANCE_TO_JUNCTION_THRESHOLD,
)
...
def process_stops(ride, legs, cur):
    ...
    no_stop_junctions = [
        jnc
        for jnc in all_ride_junctions
        if jnc[0] not in [stop.junction[0] for stop in stops]
    ]  # junctions, where cyclist did not stop
...
def find_junctions_of_stops(stops, cur):
    for stop in stops:
        ...
        if (
            junction[3] > DISTANCE_TO_JUNCTION_THRESHOLD
        ):  # stop is ignored as cyclist did not stop at a junction but somewhere on the way
            continue
...
def match_junction_to_stop(stop, cur):
    query = """    
        ...
        LIMIT 1;
        """.format(
        stop.raw_coord[0], stop.raw_coord[1]
    )
    cur.execute(query)
    return cur.fetchone()
...
def update_junction(junction, new_duration, cur):
    ...
    query = """
            ...
            WHERE id = {3};
    """.format(
        count, cum_duration, avg_duration, junction[0]
    )
    cur.execute(query)
